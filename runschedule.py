...
def add_to_startup(pythonw_path, file_path, freq_type, schedule_args, icon_name, startup):
    ...
    startup_script = os.path.abspath(__file__).replace(".py", ".pyw")  # Convert to .pyw

    args = f'"{startup_script}" "{file_path}" {freq_type} {" ".join(schedule_args)} "{icon_name.strip().lower()}" {str(startup).lower()}'
    command = f'"{pythonw_path}" {args}'

    bat_path = os.path.join(startup_dir, f"{name}_autostart.bat")

    if not os.path.exists(bat_path):
        try:
            with open(bat_path, 'w') as bat_file:
                bat_file.write(f'@echo off\n{command}\n')
            logging.info(f"Autostart script written to: {bat_path}")
        except Exception as e:
            logging.error(f"Failed to write autostart script: {e}")

# ----- ICON DISPLAYING MAIN CODE -----

def setup_tray_icon(file_path, icon_name=None, autostart_arg=None, freq_type=None, schedule_args=None):
    script_name = os.path.splitext(os.path.basename(file_path))[0]
    icons_path = os.path.join(os.path.dirname(__file__), 'Icons')

    if str(icon_name) and icon_name.strip().lower() != "none":
        menu = pystray.Menu(
            pystray.MenuItem("Details", lambda _: show_details(file_path, freq_type, schedule_args, autostart_arg)),
            pystray.MenuItem("Exit", on_quit)
        )
        icon = pystray.Icon(script_name)

        try:
            if str(icon_name).lower() == "random":
                ...
                if os.path.isabs(icon_name) and os.path.isfile(icon_name):
                    icon.icon = Image.open(icon_name)
                else:
...
if __name__ == "__main__":
    ...
    if autostart:
        try:
            add_to_startup(pythonw_path, script_path, freq_type, schedule_args, icon_arg, autostart)
        except Exception as e:
